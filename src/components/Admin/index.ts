...
export async function stakesPerDay(req: Request, res: Response): Promise<any> {
  try {
    if (!Object.keys(req.body).length) {
      throw {
        message: RES_MSG.BADREQUEST,
    ...
    const history: any = await adminService.stakePerDayHistory(
      validationSchema.value.page,
      validationSchema.value.limit
    );
...
export async function claimsPerDayHistory(req: Request, res: Response): Promise<any> {
  try {
    if (!Object.keys(req.body).length) {
      throw {
        message: RES_MSG.BADREQUEST,
    ...
    const history: any = await adminService.claimPerDayHistory(
      validationSchema.value.page,
      validationSchema.value.limit
    );
