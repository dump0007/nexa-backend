...
import { IIncomeFreezeModel } from './model';
...
class AuthValidation extends Validation {
    ...
    /**
     * @param {IIncomeFreezeModel} params
     * @returns {JoiValidationResult}
     * @memberof UserValidation
     */
    createUser(params: IIncomeFreezeModel): JoiValidationResult {
        try {
     ...
     * @memberof UserValidation
     */
    getUser(params: IIncomeFreezeModel): JoiValidationResult {
        try {
            const schema: Joi.Schema = Joi.object().keys({
                password: Joi.string().trim().required(),
                email: Joi.string().trim().email({ minDomainSegments: 2 }).required(),
